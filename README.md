...
[SimpleUNet v11a - STFT Denoising with Banana Clamping](Advanced_Speech_Denoising_STFT_128x128_hp150_SimpleUNet_v11aa.ipynb)
...
| --- |
| <p align="center"> <i> **SimpleUNet Architecture** (image generated by Gemini, with CWT as inputs) <br>  Note: inputs are STFT or CWT real and imaginary parts </i> </p> |
...
We have achieved satisfying reference results using **STFT + SimpleUNet** with the **Banana Clamping** strategy, particularly on aggressive and non-stationary noises such as helicopter. The model delivers good noise reduction while maintaining speech clarity and naturalness.

These encouraging outcomes with the STFT approach now allow us to move to the next phase of the project: **Continuous Wavelet Transform (CWT)** applied to the same dataset, aiming for improved performance on highly impulsive and non-stationary industrial signals.
